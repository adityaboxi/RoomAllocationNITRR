...
export default function RoomDashboard({ user }) {
 ...
 {isHoliday && (
 <div className="px-5 py-3 bg-white border border-slate-200 flex items-center gap-3 text-amber-900 shadow-sm animate-fadeIn">
 <div className="w-10 h-10 bg-amber-100 flex items-center justify-center text-amber-700 flex-shrink-0">
 ...
 <div className="bg-white border border-slate-200 px-8 py-5 shadow-sm space-y-5">
 {/* Top Title & Status Filter Tabs */}
 <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-4 border-b border-slate-100">
 <div className="flex items-center gap-3">
 <div className="w-10 h-10 bg-slate-50 text-slate-900 flex items-center justify-center">
 ...
 >
 <span className={`w-1.5 h-1.5 ${statusFilter === 'AVAILABLE' ? 'bg-white' : 'bg-emerald-500'}`} />
 ...
 >
 <span className={`w-1.5 h-1.5 ${statusFilter === 'OCCUPIED' ? 'bg-white' : 'bg-rose-500'}`} />
 ...
 disabled={refreshing}
 className="p-2 bg-slate-100 hover:bg-slate-200 text-slate-700 transition-all disabled:opacity-50"
 ...
 className="w-full pl-10 pr-8 py-2 bg-slate-50/50 border border-slate-200 text-xs sm:text-sm outline-none focus:bg-white focus:ring-2 focus:ring-slate-900 transition-all placeholder:text-slate-400"
 />
 {searchTerm && (
 <button
 type="button"
 ...
 className="w-full border border-slate-200 bg-slate-50/50 px-3 py-2 text-xs font-semibold text-slate-700 outline-none focus:bg-white focus:ring-2 focus:ring-slate-900 transition-all"
 >
 {floors.map((fl) => (
 ...
 className="w-full border border-slate-200 bg-slate-50/50 pl-3 pr-7 py-2 text-xs font-semibold text-slate-700 outline-none focus:bg-white focus:ring-2 focus:ring-slate-900 transition-all appearance-none"
 >
 ...
 filterProjector
 ? 'bg-slate-900 text-white border-indigo-600 shadow-sm'
 ...
 type="button"
 onClick={handleResetFilters}
 className="mt-4 px-5 py-3 bg-slate-50 text-slate-900 text-xs font-bold hover:bg-indigo-100 transition-colors inline-block"
 >
 Reset All Filters
 </button>
 )}
 ...
 <div
 key={roomId}
 className={`bg-white border px-8 py-5 shadow-sm flex flex-col justify-between transition-all duration-200 ${
 isMyOngoingClass
 ? 'border-indigo-500 ring-2 ring-indigo-200 shadow-md bg-slate-50/20'
 : available
 ...
 {isHoliday ? (
 <span className="px-2.5 py-1 text-xs font-bold bg-amber-100 text-amber-800">
 Holiday / Closed
 </span>
 ) : available ? (
 <span className="px-2.5 py-1 text-xs font-bold bg-emerald-100 text-emerald-800 flex items-center gap-1.5">
 <span className="w-1.5 h-1.5 bg-emerald-500 animate-pulse" />
 <span>Free Now</span>
 </span>
 ) : isMyOngoingClass ? (
 <span className="px-3 py-1 text-xs font-extrabold bg-slate-900 text-white shadow-sm flex items-center gap-1.5 animate-pulse">
 <GraduationCap className="w-3.5 h-3.5" />
 <span>In Class</span>
 </span>
 ) : (
 <span className="px-2.5 py-1 text-xs font-bold bg-rose-100 text-rose-800 flex items-center gap-1.5">
 ...
 {!available && occupancy && (
 <div
 className={`mt-2 p-2.5 text-xs font-medium border ${
 isMyOngoingClass
 ...
 {room.hasProjector && (
 <span className="bg-slate-100 text-slate-700 px-2 py-0.5 font-medium">
 Projector
 </span>
 )}
 {room.hasAC && (
 ...
 className="text-xs font-semibold text-slate-900 hover:text-indigo-800"
 >
