...
export default function ReviewPopup({ booking, onSubmit, onSkip }) {
 ...
 >
 <div
 className="bg-white shadow-lg max-w-md w-full px-8 py-5 sm:p-7 relative border border-slate-200"
 onClick={(e) => e.stopPropagation()}
 >
 <button
 type="button"
 onClick={onSkip}
 className="absolute top-4 right-4 text-slate-400 hover:text-slate-700 w-8 h-8 bg-slate-100 flex items-center justify-center transition-colors"
 disabled={loading}
 >
 <X className="w-4 h-4" />
 </button>
 ...
 <div className="w-10 h-10 bg-amber-50 border border-amber-100 flex items-center justify-center text-amber-500">
 ...
 <p className="text-xs sm:text-sm text-slate-600 mb-4 bg-slate-50 p-3 border border-slate-100 leading-relaxed">
 How was your experience in <strong>{roomName}</strong> on <strong>{booking.date}</strong> ({booking.startTime} - {booking.endTime})?
 </p>

 {error && (
 <div className="bg-white border border-slate-200 text-rose-800 p-3 mb-4 text-xs font-medium whitespace-pre-line">
 {error}
 </div>
 )}
 ...
 className="w-full border border-slate-200 p-3 text-xs sm:text-sm focus:ring-2 focus:ring-slate-900 focus:bg-white bg-slate-50/50 outline-none transition-all"
 rows="3"
 ...
 className="flex-1 bg-slate-900 text-white py-2.5 text-xs font-bold hover:bg-slate-800 transition-all shadow-sm disabled:opacity-50 flex items-center justify-center gap-1.5"
 >
 {loading ? (
 <>
 ...
 onClick={onSkip}
 disabled={loading}
 className="px-5 py-3.5 bg-slate-100 text-slate-700 text-xs font-semibold hover:bg-slate-200 transition-colors disabled:opacity-50"
 >
 Skip
 </button>
 </div>
