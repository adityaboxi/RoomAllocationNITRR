...
api.interceptors.response.use(
  ...
  (error) => {
    // Detect expired token or deactivated account and clear local storage
    ...
    const message =
      error.response?.data?.message || error.message || 'Server request failed';
...
export const getRoomsByFloor = () =>
  api.get('/api/rooms/floors');

export const getRoomsByBuilding = () =>
  api.get('/api/rooms/buildings');

export const getRoomsByDepartment = (department) =>
  api.get(`/api/rooms/department/${department}`);
...
export const getRoomAvailability = (roomId, day, time, date) =>
  api.get(`/api/rooms/${roomId}/availability`, {
    params: { day, time, date },
  });

// ---------- TIMETABLE ENDPOINTS ----------
export const getTimetable = (params = {}) =>
  api.get('/api/timetable', { params });
...
export const getTimetableByRoom = (roomId) =>
  api.get(`/api/timetable/room/${roomId}`);

export const replaceTimetable = (data) =>
  api.post('/api/timetable', data);

export const updateTimetableEntry = (id, data) =>
  api.put(`/api/timetable/${id}`, data);
...
/**
 * Flexible room lock helper supporting object payload or separate arguments
 */
...
export const unlockRoom = (lockId) =>
  api.post('/api/bookings/unlock', { lockId });

export const getBookingsByRoom = (roomId) =>
  api.get(`/api/bookings/room/${roomId}`);

export const getBookingsByFaculty = (facultyEmail) =>
  api.get(`/api/bookings/faculty/${facultyEmail}`);

// ---------- STATS ENDPOINTS ----------
export const getDepartmentStats = (department) =>
  api.get(`/api/stats/department/${department}`);
...
export const getRoomReviews = (roomId) =>
  api.get(`/api/reviews/room/${roomId}`);
